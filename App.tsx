...
const InteractionGroup: React.FC<InteractionGroupProps> = ({
  ...
  <div className={`flex flex-col gap-6 w-full max-w-[840px] mx-auto transition-opacity duration-1000 ${isVisible ? 'opacity-100 fade-in-content' : 'opacity-0'}`}>
        ...
        <div className="flex flex-wrap justify-center gap-3">
          {SUGGESTED_QUESTIONS.slice(0, 3).map((q, idx) => (
            <button
              ...
              className="whitespace-nowrap flex h-11 items-center justify-center rounded-full border border-gray-300 dark:border-[#2A2A2A] bg-transparent hover:bg-gray-100 dark:hover:bg-[#1A1A1A] px-6 hover:border-[#149383] hover:text-[#149383] transition-all duration-200 text-sm font-medium text-gray-600 dark:text-gray-400"
            >
              {q}
            </button>
          ))}
    ...
    <div className="relative group w-full">
      <div className="flex items-center rounded-[24px] bg-white dark:bg-[#1A1A1A] border border-gray-200 dark:border-[#2A2A2A] p-2 pr-3 transition-all focus-within:ring-2 focus-within:ring-[#149383]/30 focus-within:border-[#149383]/50 shadow-sm">
        <input
          className="flex-1 bg-transparent border-none focus:ring-0 px-5 py-4 text-lg placeholder:text-gray-400 dark:placeholder:text-gray-600 text-gray-900 dark:text-white"
        ...
        <button
          ...
          className="flex h-12 w-12 items-center justify-center rounded-[18px] bg-[#149383] text-white hover:brightness-110 disabled:opacity-30 disabled:grayscale transition-all shadow-md active:scale-95"
        >
    ...
    <div className="flex flex-col items-center gap-4">
      <span className="text-gray-400 dark:text-[#555] text-[10px] font-bold tracking-[0.2em] uppercase">Contact Me:</span>
...
const App: React.FC = () => {
  ...
  return (
    <div className={`flex flex-col h-screen overflow-hidden font-sans transition-colors duration-300 ${theme === 'dark' ? 'bg-[#121212]' : 'bg-[#F9F8F6]'}`}>
      ...
      <button
        onClick={toggleTheme}
        className="fixed top-6 right-6 z-50 w-11 h-11 flex items-center justify-center rounded-full bg-[#242424] border border-[#333] hover:border-[#149383] transition-all shadow-md group"
      >
        <span className="material-symbols-outlined text-[22px] text-white/80 group-hover:text-[#149383]">
      ...
      >
        <div className={`min-h-full flex flex-col items-center transition-all duration-700 ${!hasMessages ? 'justify-center py-20' : 'justify-start pt-16'}`}>
          ...
          <div className={`w-full max-w-[960px] px-6 flex flex-col items-center text-center transition-all duration-500 ${!hasMessages ? 'mb-12' : 'mb-12 opacity-80 scale-90'}`}>
            <h1 className={`text-gray-900 dark:text-white text-4xl md:text-[56px] font-extrabold tracking-tight leading-tight min-h-[1.2em] ${introPhase === 'title' ? 'cursor-active' : ''}`}>
              {typedTitle}
            </h1>
            <p className={`mt-4 text-gray-500 dark:text-[#9DA3A0] text-lg md:text-xl font-medium min-h-[1.5em] ${introPhase === 'subtitle' ? 'cursor-active' : ''}`}>
              {typedSubtitle}
            </p>
          </div>

          {hasMessages && (
            <div className="w-full max-w-[960px] px-6 space-y-10 pb-[210px]">
              {messages.map((msg) => (
                <div
                  key={msg.id}
                ...
                >
                  <div
                    className={`
                      max-w-[85%] px-7 py-5 rounded-[2rem] shadow-sm border
                      ${msg.role === 'user'
                    ...
                    `}
                  >
                    <div className={`message-stream text-lg leading-relaxed whitespace-pre-wrap ${msg.isGenerating ? 'cursor-active' : ''}`}>
                      {msg.content}
                    </div>
                  </div>
                </div>
      ...
      {hasMessages && (
        <div className={`fixed bottom-0 left-0 w-full z-40 pt-12 pb-10 transition-colors duration-300 pointer-events-none ${theme === 'dark' ? 'bg-gradient-to-t from-[#121212] via-[#121212] to-transparent' : 'bg-gradient-to-t from-[#F9F8F6] via-[#F9F8F6] to-transparent'}`}>
