...
const InteractionGroup: React.FC<InteractionGroupProps> = ({
  ...
  <div className={`flex flex-col gap-6 w-full max-w-[840px] mx-auto transition-opacity duration-1000 ${isVisible ? 'opacity-100 fade-in-content' : 'opacity-0'}`}>
    ...
    <div className={`flex flex-col items-center ${hasMessages ? 'gap-1 sm:gap-4' : 'gap-2 sm:gap-4'}`}>
      <span className={`text-gray-400 dark:text-[#555] ${hasMessages ? 'text-[8px] sm:text-[10px]' : 'text-[10px] sm:text-xs'} font-bold tracking-[0.2em] uppercase`}>Contact Me:</span>
      <div className={`flex items-center ${hasMessages ? 'gap-4 sm:gap-10' : 'gap-6 sm:gap-10'}`}>
        <a
          ...
          title="LinkedIn"
        >
          <svg className={`w-5 h-5 ${hasMessages ? 'sm:w-7 sm:h-7' : 'sm:w-6 sm:h-6'} fill-current`} viewBox="0 0 24 24">
        ...
        <a
          ...
          title="Email"
        >
          <span className={`material-symbols-outlined text-[24px] ${hasMessages ? 'sm:text-[32px]' : 'sm:text-[28px]'}`}>mail</span>
        </a>
      </div>
    </div>
  </div>
